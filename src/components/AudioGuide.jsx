  ...
  if (!supported) {
    return (
      <div className={styles.unsupported} data-audio-skip="true" role="status">
        <Headphones size={17} /> Audio read-aloud is not available in this browser.
      </div>
    );
  }
  ...
  return (
    <div className={styles.guide} data-audio-skip="true" aria-label="Audio guide controls">
      ...
      <div className={styles.controls}>
        <button type="button" className={styles.primary} onClick={() => speak()} aria-label={label}>
          <Play size={16} fill="currentColor" /> {label}
        </button>
        <button type="button" className={styles.control} onClick={togglePause} disabled={!isActive} aria-label={status === 'paused' ? 'Resume audio' : 'Pause audio'}>
          {status === 'paused' ? <Play size={16} /> : <Pause size={16} />} <span>{status === 'paused' ? 'Resume' : 'Pause'}</span>
        </button>
        <button type="button" className={styles.control} onClick={replay} disabled={!lastText} aria-label="Replay audio">
          <RotateCcw size={16} /> <span>Replay</span>
        </button>
        <button type="button" className={styles.control} onClick={stop} disabled={!isActive} aria-label="Stop audio">
          <Square size={14} fill="currentColor" /> <span>Stop</span>
        </button>
        <button type="button" className={styles.rate} onClick={changeRate} aria-label={`Audio speed ${RATES[rateIndex]} times. Tap to change.`}>
          <Gauge size={16} /> {RATES[rateIndex]}x
        </button>
      </div>
    </div>
  );
